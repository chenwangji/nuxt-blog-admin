      ...
      <el-col :span="16">
        ...
        <el-form-item
          ...
          <markdown-editor
            ...
            :configs="configs"
            highlight
            preview-class="mardown-body"></markdown-editor>
        </el-form-item>
        <el-form-item
...
export default class Release extends Vue {
  ...
  private submitForm (formName: string): void {
    (this.$refs[formName] as HTMLFormElement).validate(async (valid: boolean): Promise<boolean> => {
      if (valid) {
        const dispatchType = this.id ? 'article/putArt' : 'article/postArt'
        let res: Ajax.AjaxResponse = await this.$store.dispatch(dispatchType, { ...this.form })
  ...
  private beforeUpload (file: File): boolean {
    ...
    if (!isJPG) {
      error('上传的图片只能是 JPG/PNG 格式!')
    }
    if (!isLt10M) {
      error('上传的图片大小不能超过 10M!')
    }
  ...
  private handleSuccess (): void {
    this.form.thumb = QN_URL + this.qn.key
  }
  ...
  private async created (): Promise<void> {
    ...
    if (id) {
      this.id = id
      this.$store.dispatch('article/getArt', { _id: id })
    }
  }
}
</script>
