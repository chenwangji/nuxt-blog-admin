...
export default class Links extends Vue {
  ...
  private get fetch (): boolean {
    return this.$store.state.links.fetch
  }
  private get posting (): boolean {
    return this.$store.state.links.posting
  }
  private get total (): number {
    return this.$store.state.links.total
  }
  ...
  private async changeState (
    ...
    await this.$store.dispatch('links/patchLink', {
      _id: row._id,
      state: code
    })
  }
  ...
  private deleteLink (row: StoreState.Link): void {
    this.$confirm('确定删除此数据吗？', '提示', {
      ...
      const res = await this.$store.dispatch('links/deleteLink', { _id: row._id })
  ...
  private async getData (): Promise<void> {
    const res: Ajax.AjaxResponse = await this.$store.dispatch('links/getLinks', {
